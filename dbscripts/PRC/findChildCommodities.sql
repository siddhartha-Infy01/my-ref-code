...
CREATE DEFINER=`esoko`@`%` PROCEDURE `findChildCommodities`(parentid VARCHAR(25))
BEGIN
  ...
  insert into _result10(select commodity_id from commodities where parent_id= parentid);

...
-- --------------------------------------------------------------------------------
drop procedure queryAdmins;
DELIMITER $$

CREATE DEFINER=`esoko`@`%` PROCEDURE `queryAdmins`(nwid VARCHAR(25),commodityids VARCHAR(25),locationids VARCHAR(25),occupationids VARCHAR(25))
BEGIN
...
primary key (people_id));
   ...
   if commodityids !='' and  locationids !='' and occupationids !='' then
	...
	INSERT IGNORE INTO _tmp_people SELECT p.people_id,p.first_name,p.last_name,p.nickname,p.gender,p.town,p.country,p.currency_id,p.language_id,p.msisdn,p.email,'Y',p.default_network_id FROM people p, Networks n,user_commodities u, user_group g WHERE p.default_network_id=nwid and p.people_id=u.user_id and p.default_network_id=n.network_id and n.record_status='A' and u.commodity_id in (select * from _resultComm) AND p.record_status='A' and p.default_network_id=g.network_id and g.group_id='Administrator' and p.people_id=g.user_id and p.default_network_id=u.network_id;        
		
	INSERT IGNORE INTO _tmp_people1 SELECT p.people_id,p.first_name,p.last_name,p.nickname,p.gender,p.town,p.country,p.currency_id,p.language_id,p.msisdn,p.email,'Y',p.default_network_id FROM people p, Networks n,user_locations u, user_group g WHERE p.default_network_id=nwid and p.people_id=u.user_id and p.default_network_id=n.network_id and n.record_status='A' and  u.location_id in (select * from _resultLoc) AND p.record_status='A' and p.default_network_id=g.network_id and g.group_id='Administrator' and p.people_id=g.user_id and p.default_network_id=u.network_id;        
	    
	INSERT IGNORE INTO _tmp_people2 SELECT p.people_id,p.first_name,p.last_name,p.nickname,p.gender,p.town,p.country,p.currency_id,p.language_id,p.msisdn,p.email,'Y',p.default_network_id FROM people p, Networks n,user_occupations u, user_group g WHERE p.default_network_id=nwid and p.people_id=u.user_id and p.default_network_id=n.network_id and n.record_status='A' and  u.occupation_id in (select * from _resultOcc) AND p.record_status='A' and p.default_network_id=g.network_id and g.group_id='Administrator' and p.people_id=g.user_id and p.default_network_id=u.network_id;        
    ...
    INSERT IGNORE INTO _tmp_people SELECT p.people_id,p.first_name,p.last_name,p.nickname,p.gender,p.town,p.country,p.currency_id,p.language_id,p.msisdn,p.email,'Y',p.default_network_id FROM people p, Networks n, user_group g WHERE p.default_network_id=nwid and p.default_network_id=n.network_id and n.record_status='A' and p.record_status='A' and p.default_network_id=g.network_id and g.group_id='Administrator' and p.people_id=g.user_id;            

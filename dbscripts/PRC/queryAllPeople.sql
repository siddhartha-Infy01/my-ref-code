...
-- --------------------------------------------------------------------------------
drop procedure queryAllPeople;
DELIMITER $$

CREATE DEFINER=`esoko`@`%` PROCEDURE `queryAllPeople`(nwid VARCHAR(25),commodityids VARCHAR(25),locationids VARCHAR(25),occupationids VARCHAR(25))
BEGIN
...
`default_network_id` VARCHAR(25) NOT NULL DEFAULT '',
 PRIMARY KEY (`people_id`));
   ...
   if commodityids !='' and  locationids !='' and occupationids !='' then
	...
	INSERT IGNORE INTO _tmp_people SELECT p.people_id,p.first_name,p.last_name,p.nickname,p.gender,p.town,p.country,p.currency_id,p.language_id,p.msisdn,p.email,'Y',p.default_network_id FROM people p, Networks n,user_commodities u WHERE p.default_network_id=nwid and p.people_id=u.user_id and p.default_network_id=n.network_id and n.record_status='A' and u.commodity_id in (select * from _resultComm) AND p.record_status='A' and p.default_network_id=u.network_id;
	INSERT IGNORE INTO _tmp_people1 SELECT p.people_id,p.first_name,p.last_name,p.nickname,p.gender,p.town,p.country,p.currency_id,p.language_id,p.msisdn,p.email,'Y',p.default_network_id FROM people p, Networks n,user_locations u WHERE p.default_network_id=nwid and p.people_id=u.user_id and p.default_network_id=n.network_id and n.record_status='A' and u.location_id in (select * from _resultLoc) AND p.record_status='A' and p.default_network_id=u.network_id;
	INSERT IGNORE INTO _tmp_people2 SELECT p.people_id,p.first_name,p.last_name,p.nickname,p.gender,p.town,p.country,p.currency_id,p.language_id,p.msisdn,p.email,'Y',p.default_network_id FROM people p, Networks n,user_occupations u WHERE p.default_network_id=nwid and p.people_id=u.user_id and p.default_network_id=n.network_id and n.record_status='A' and u.occupation_id in (select * from _resultOcc) AND p.record_status='A' and p.default_network_id=u.network_id;
	
	INSERT IGNORE INTO _tmp_people SELECT p.people_id,p.first_name,p.last_name,p.nickname,p.gender,p.town,p.country,p.currency_id,p.language_id,p.msisdn,p.email,'N',p.default_network_id FROM people p, Networks n,user_commodities u WHERE p.default_network_id=n.network_id and p.people_id=u.user_id and n.record_status='A' and u.commodity_id in (select * from _resultComm) and  p.default_network_id in (select * from _tmpshare) and p.default_network_id=u.network_id;    	       	
	INSERT IGNORE INTO _tmp_people1 SELECT p.people_id,p.first_name,p.last_name,p.nickname,p.gender,p.town,p.country,p.currency_id,p.language_id,p.msisdn,p.email,'N',p.default_network_id FROM people p, Networks n,user_locations u WHERE p.default_network_id=n.network_id and p.people_id=u.user_id and n.record_status='A' and  u.location_id in (select * from _resultLoc) and  p.default_network_id in ( select * from _tmpshare)  and p.default_network_id=u.network_id;    	        		
	INSERT IGNORE INTO _tmp_people2 SELECT p.people_id,p.first_name,p.last_name,p.nickname,p.gender,p.town,p.country,p.currency_id,p.language_id,p.msisdn,p.email,'N',p.default_network_id FROM people p, Networks n,user_occupations u WHERE p.default_network_id=n.network_id and p.people_id=u.user_id and n.record_status='A' and  u.occupation_id in (select * from _resultOcc) and  p.default_network_id in ( select * from _tmpshare) and p.default_network_id=u.network_id;    	       			
	
    SELECT * FROM _tmp_people where people_id in ( select people_id from _tmp_people1 where people_id in (select people_id from _tmp_people2));
	...
	SELECT * FROM _tmp_people1 where people_id in ( select people_id from _tmp_people2);

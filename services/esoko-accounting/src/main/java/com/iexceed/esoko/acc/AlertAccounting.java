...
@Component
public class AlertAccounting extends AbstractAccounting {
	@Autowired
	...
	public String doAccounting(Accounting accounting) {
		log.info("Entered Alert Accounting");
		...
		Forex forex = null;
		Map<String, List<Map<ACCROLE, COSTROLE>>> ALERT = ROLETOHEAD.ALERT;
		...
		List<Map<ACCROLE, COSTROLE>> debits = ALERT.get(ROLETOHEAD.ALDEBIT);
			...
			for (ACCROLE accrole : map.keySet()) {
				...
				if (accrole.getACCRole().equals(ACCROLE.D_NWK_ACC.getACCRole())
					...
					break;
						...
						ACCROLE.D_USR_ACC.getACCRole())
						&& accounting.getDebitType().equals("U")) {
					log.debug("Will debit User Account");
					...
					break;
						...
						ACCROLE.D_RCP_ACC.getACCRole())
						&& accounting.getDebitType().equals("R")) {
					log.debug("Will debit Recipient Account");
					system_transaction = new System_transaction();
					netTransactionPK = new System_transactionPK();
					netTransactionPK.setTransactionId(++i);
					netTransactionPK.setTransactionRefNo(trnRefNo);
					...
					system_transaction
							.setAccountNo(sys_user_acc.getAccountNo());
					log.info("Cost Role::" + map.get(accrole).getCostRole());
					log.info("Actual Cost ROle::" + COSTROLE.COST);
		...
		List<Map<ACCROLE, COSTROLE>> credits = ALERT.get(ROLETOHEAD.ALCREDIT);

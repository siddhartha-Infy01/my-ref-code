...
count(*) as total
 FROM message_delivary_Details a where message_type in ('BO','PA','W','CT') and network_id=? and created_Ts between ? and ?  group by schedule_id,date(created_ts) order by created_Ts;